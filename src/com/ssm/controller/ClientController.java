...
public class ClientController {
	...
	public String clientContact(Model model,HttpSession session) {
	...
	public String clientContact(Contact contact,Model model,HttpSession session) {
	...
	public String clientFeedback(HttpSession session,Model model) {
		Integer uid = ((User)session.getAttribute("loginer")).getUid();
		//获取用户名
		User user = userService.findUserById(uid);
		model.addAttribute("user",user);
	...
	public String clientFeedback(feedback feed,Model model,HttpSession session) {
		// 循环获取商品分类
				List<Category> categories = serverService.findAllCategories();
				model.addAttribute("categories", categories);
